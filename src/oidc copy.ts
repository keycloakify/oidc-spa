...
>(params: ParamsOfCreateOidc<DecodedIdToken>): Promise<Oidc<DecodedIdToken>> {
    ...
    const oidc = id<Oidc.LoggedIn<DecodedIdToken>>({
        ...
        "enableAutoLogout": ({ countdown } = {}) => {
            assert(
                stopAutoRenew !== undefined,
                "disableAutoLogout() should be called before calling enableAutoLogout() again"
            );

            stopAutoRenew();

            const { stopRenewOnUserActivity } = startRenewOnUserActivity({ countdown });

            const disableAutoLogout = () => {
                stopRenewOnUserActivity();
                stopAutoRenew = startAutoRenew().stopAutoRenew;
            };
    ...
    const { startAutoRenew, startRenewOnUserActivity } = (() => {
        ...
        const startRenewOnUserActivity = (params: {
            countdown:
                | {
                      startTickAtSecondsLeft: number;
                      tickCallback: (params: { secondsLeft: number }) => void;
            ...
            const stopRenewOnUserActivity = () => {};

            return { stopRenewOnUserActivity };
        };
        ...
        const startAutoRenew = () => {
            ...
            // If the token expiration time is less than 25 seconds we refresh the token when
            // only 1/10 of the token time is left.
            const scheduleTimeoutMsBeforeExpiration = Math.min(25 * 1000, getMsBeforeExpiration() * 0.1);
            ...
            (function scheduleRenew() {
                timer = setTimeout(async () => {
                    ...
                    scheduleRenew();
                }, getMsBeforeExpiration() - scheduleTimeoutMsBeforeExpiration);
            ...
            return { stopAutoRenew };
        };

        return {
            startRenewOnUserActivity,
            startAutoRenew
        };
    })();

    let stopAutoRenew: (() => void) | undefined = startAutoRenew().stopAutoRenew;
