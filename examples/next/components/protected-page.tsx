...
import { fetchWithAuth, useOidc } from "@/lib/oidc";
...
export function ProtectedPage() {
    ...
    const { demoPosts, errorMessage } = useDemoPosts();
    ...
    useEffect(() => {
        ...
        fetchWithAuth("https://jsonplaceholder.typicode.com/posts?_limit=4")
            .then(async response => {
                ...
                return (await response.json()) as DemoPost[];
            })
            .then(posts => {
                if (!isCancelled) {
                    setDemoPosts(posts);
            ...
            .catch(error => {
                if (!isCancelled) {
                    setErrorMessage(
                        error instanceof Error ? error.message : "Failed to load demo posts"
                    );
                }
            });
